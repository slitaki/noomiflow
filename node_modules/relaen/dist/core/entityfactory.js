...
class EntityFactory {
    ...
    static addEntityConfig(entity, tblName, schema) {
        ...
        if (EntityFactory.entityConfigs.has(entityName)) {
            let cfg = EntityFactory.entityConfigs.get(entityName);
    ...
    static addPKey(entityName, propName, cfg) {
        ...
        if (cfg.generator === 'table' && !cfg.keyName || cfg.generator === 'sequence' && !cfg.seqName) {
            throw errorfactory_1.ErrorFactory.getError("0050", [entityName]);
        }
        let ecfg = this.entityConfigs.get(entityName);
        ecfg.setId(cfg);
    }
    /**
     * 设置版本号字段名
            ...
            for (const dirent of dir) {
                if (dirent.isDirectory()) {
                    ...
                    if (reg.test(dirent.name)) {
                        await Promise.resolve().then(() => require(pathMdl.resolve(dirPath, dirent.name)));
                    }
                }
            }
        }
    ...
    static setIdValue(entity, value) {
        let cfg = this.getEntityConfig(entity.constructor.name);
        if (cfg) {
